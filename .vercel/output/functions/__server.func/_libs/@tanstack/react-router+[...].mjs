        ...
        case 0: {
          ...
          if (caseSensitive) {
            ...
            if (existingNode) nextNode = existingNode;
              ...
              var _node2;
              (_node2 = node).staticInsensitive ??
                (_node2.staticInsensitive = /* @__PURE__ */ new Map());
              const next = createStaticNode(route.fullPath ?? route.from);
              next.parent = node;
              next.depth = depth;
              nextNode = next;
    ...
    if (node.wildcard)
      for (let i = node.wildcard.length - 1; i >= 0; i--) {
        ...
        if (prefix) {
          ...
          if (
            !(
              segment.caseSensitive ? part : (lowerPart ?? (lowerPart = part.toLowerCase()))
            ).startsWith(prefix)
          )
            continue;
        }
          ...
          try {
            ...
            if (meta && validated) {
              for (const key in validated)
                if (!(key in result))
                  (meta.defaulted || (meta.defaulted = /* @__PURE__ */ new Map())).set(
                    key,
                    validated[key],
                  );
            }
...
export {
  ...
  createRouter as u,
  createInlineCssPlaceholderAsset as v,
  executeRewriteInput as w,
  getStylesheetHref as x,
  createInlineCssStyleAsset as y,
};
